...
enum class AssertType { Throw, Continue };

struct AssertException : std::exception {
  AssertException( std::string message ) : m_message( std::move( message ) ) {}
  ...
  bool ParseCommandLine( int argc, const char* argv[] ) {
    for ( int index = 1; index < argc; ++index ) {
      ...
      if ( option_name == "h" || option_name == "-help" || option_name == "?" ) {
        ...
        std::cout << "        --adapter_info:    Output additional details for test adapters"
                  << std::endl;
        return false;
      }
...
struct ILogger {
  ...
  virtual void AssertFailed( AssertType type, const AssertLocation& location,
...
struct ConsoleLogger : ILogger {
  ...
  void AssertFailed( AssertType type, const AssertLocation& location,
 ...
 private:
  ConsoleLogger( const RunOptions* run_options ) : m_run_options( run_options ) {}
...
struct TestRegistry {
 ...
 public:
  ...
  struct AutoReg {
    AutoReg() { TestRegistry::Add<TTestCase>(); }
  };
 ...
 private:
  ...
  static bool ShouldRunTest( const RunOptions* options, const std::string_view& test_name,
...
inline std::optional<AssertException> Close( double left, double right,
...
struct SectionLock {
 ...
 private:
  ...
  SectionLock( SectionLock&& other ) : m_logger00( std::move( other.m_logger00 ) ) {}
...
struct CommonFixture {
  CommonFixture( ILoggerPtr logger ) : m_logger( std::move( logger ) ) {}

  bool HaveChecksFailed() const { return m_check_has_failed; }

 protected:
  SectionLock EnterSection( std::string text ) { return SectionLock( text, m_logger ); }

  void HandleAssert( AssertType behavior, const AssertLocation& location,
  ...
  static constexpr auto make_tags_array( TArgs&&... tags ) {
    return std::array<std::string_view, sizeof...( TArgs )>{std::forward<TArgs>( tags )...};
  }
...
using TestFixtureBase =
    std::conditional_t<std::is_base_of_v<CommonFixture, T>, T, TestFixtureBaseImpl<T>>;
...
#define SECTION( Text ) \
  if ( auto _CPPUTF_NEXT_SECTION_LOCK_NAME = EnterSection( "Section: " Text ); true )
#define SCENARIO( Text ) \
  if ( auto _CPPUTF_NEXT_SECTION_LOCK_NAME = EnterSection( "Scenario: " Text ); true )
#define GIVEN( Text ) \
  if ( auto _CPPUTF_NEXT_SECTION_LOCK_NAME = EnterSection( "Given: " Text ); true )
#define AND( Text ) if ( auto _CPPUTF_NEXT_SECTION_LOCK_NAME = EnterSection( "And: " Text ); true )
#define WHEN( Text ) \
  if ( auto _CPPUTF_NEXT_SECTION_LOCK_NAME = EnterSection( "When: " Text ); true )
#define THEN( Text ) \
  if ( auto _CPPUTF_NEXT_SECTION_LOCK_NAME = EnterSection( "Then: " Text ); true )
...
#define REQUIRE( Expression )                                           \
  CppUnitTestFramework::CommonFixture::HandleAssert(                    \
      ...
      CppUnitTestFramework::Assert::IsTrue( static_cast<bool>( Expression ), #Expression ) )
...
#define REQUIRE_FALSE( Expression )                                     \
  CppUnitTestFramework::CommonFixture::HandleAssert(                    \
      CppUnitTestFramework::AssertType::Throw, _CPPUTF_ASSERT_LOCATION, \
      CppUnitTestFramework::Assert::IsFalse( static_cast<bool>( Expression ), #Expression ) )
#define REQUIRE_EQUAL( Left, Right )                                    \
  CppUnitTestFramework::CommonFixture::HandleAssert(                    \
      CppUnitTestFramework::AssertType::Throw, _CPPUTF_ASSERT_LOCATION, \
      CppUnitTestFramework::Assert::AreEqual( ( Left ), ( Right ) ) )
#define REQUIRE_NULL( Expression )                                      \
  CppUnitTestFramework::CommonFixture::HandleAssert(                    \
      CppUnitTestFramework::AssertType::Throw, _CPPUTF_ASSERT_LOCATION, \
      CppUnitTestFramework::Assert::IsNull( ( Expression ), #Expression ) )
#define REQUIRE_NOT_NULL( Expression )                                  \
  CppUnitTestFramework::CommonFixture::HandleAssert(                    \
      CppUnitTestFramework::AssertType::Throw, _CPPUTF_ASSERT_LOCATION, \
      CppUnitTestFramework::Assert::IsNotNull( ( Expression ), #Expression ) )
#define REQUIRE_THROW( ExceptionType, Expression )                      \
  CppUnitTestFramework::CommonFixture::HandleAssert(                    \
      CppUnitTestFramework::AssertType::Throw, _CPPUTF_ASSERT_LOCATION, \
      CppUnitTestFramework::Assert::Throws<ExceptionType>( [&] { Expression; } ) )
#define REQUIRE_NO_THROW( Expression )                                  \
  CppUnitTestFramework::CommonFixture::HandleAssert(                    \
      CppUnitTestFramework::AssertType::Throw, _CPPUTF_ASSERT_LOCATION, \
      CppUnitTestFramework::Assert::NoThrow( [&] { Expression; } ) )
#define REQUIRE_CLOSE( Left, Right, Percentage )                        \
  CppUnitTestFramework::CommonFixture::HandleAssert(                    \
      CppUnitTestFramework::AssertType::Throw, _CPPUTF_ASSERT_LOCATION, \
      CppUnitTestFramework::Assert::Close( ( Left ), ( Right ), ( Percentage ) ) )
#define REQUIRE_CLOSE_FRACTION( Left, Right, Fraction )                 \
  CppUnitTestFramework::CommonFixture::HandleAssert(                    \
      CppUnitTestFramework::AssertType::Throw, _CPPUTF_ASSERT_LOCATION, \
      CppUnitTestFramework::Assert::CloseFraction( ( Left ), ( Right ), ( Fraction ) ) )

#define CHECK( Expression )                                                \
  CppUnitTestFramework::CommonFixture::HandleAssert(                       \
      CppUnitTestFramework::AssertType::Continue, _CPPUTF_ASSERT_LOCATION, \
      CppUnitTestFramework::Assert::IsTrue( static_cast<bool>( Expression ), #Expression ) )
...
int main( int argc, const char* argv[] ) {
  ...
  bool success = CppUnitTestFramework::TestRegistry::Run(
      &options, CppUnitTestFramework::ConsoleLogger::Create( &options ) );
